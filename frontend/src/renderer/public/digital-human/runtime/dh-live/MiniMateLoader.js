...
// Vendored from DH_live's web_demo. The DOMContentLoaded listener above never
// fires when this script is injected into an already-loaded SPA page, so
// expose init() explicitly for callers (see DhLiveAvatar.tsx) to invoke once
// the required canvas/video DOM nodes exist.
window.__dhLiveMiniInit = () => init();
...
async function qtLoad(config)
{
    ...
    config.locateFile = filename =>
    {
        ...
        // Vendored fix: the demo's own page lived next to DHLiveMini.wasm, so a
        // bare filename happened to resolve correctly there. In this app the
        // page is served from the site root while the runtime lives under
        // /digital-human/runtime/dh-live/, so the bare filename resolved
        // against location.origin instead and silently fetched index.html.
        if (originalLocatedFilename.endsWith('.wasm') && !originalLocatedFilename.includes('/'))
            return `/digital-human/runtime/dh-live/${originalLocatedFilename}`;
