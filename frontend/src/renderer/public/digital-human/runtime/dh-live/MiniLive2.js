// ==================== 重要配置项 ====================
// Asset dir is overridable by the React wrapper (DhLiveAvatar.tsx) via
// window.__dhLiveAssetDir, set before this script is injected, so different
// avatar_renderer.asset_id values can point at different vendored folders.
const __dhLiveAssetDir = window.__dhLiveAssetDir || '/digital-human/test_avatar';
let CONFIG = {
    showFPS: false,              // 是否显示 FPS
    chromaKeyEnabled: false,     // 是否开启绿幕扣除
    backgroundVideoSrc: __dhLiveAssetDir + "/idle.mp4",  // 背景视频路径（开启绿幕扣除时使用）
    videoSrc: __dhLiveAssetDir + "/idle.mp4",                // 默认视频文件路径
    dataSrc: __dhLiveAssetDir + "/combined_data.json.gz",   // 默认数据文件路径
    // 绿幕抠图参数配置
    chromaKey: {
...
//            gl.bindTexture(gl.TEXTURE_2D, null);
        ...
        image.src = '/digital-human/runtime/dh-live/bs_texture_halfFace.png';
}
