...
function General({ onSave, onCancel }: GeneralProps): JSX.Element {
  ...
  const microphones = createListCollection({
    items: audioInputDevices.map(({ deviceId, label }) => ({
      label,
      value: deviceId,
    })),
  });
  ...
  return (
    <Stack {...settingStyles.common.container}>
      ...
      <SelectField
        label={i18n.language === "zh" ? "麦克风" : "Microphone"}
        value={[selectedAudioInputDeviceId]}
        onChange={(value) => setSelectedAudioInputDeviceId(value[0] || "default")}
        collection={microphones}
        placeholder={i18n.language === "zh" ? "选择物理麦克风" : "Choose a physical microphone"}
      />
