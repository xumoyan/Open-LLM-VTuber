...
Docker 公开 `221.216.142.88:18081`，但只有先发送 `MOBILE_ACCESS_TOKEN` 的客户端才能创建 Qwen 会话。该口令只适用于单设备私测，不能用于商店发布。
...
`sync:android-ip-test` 只为 Debug 允许 `http/ws://221.216.142.88:18081`。不要用它构建 release。
